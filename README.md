...
**\***additional BioProject accession ids, of cognate Genbank accessions present in accessions_filtered_refseq_gb.tsv, but not found in accessions_filtered.tsv. This could be for the following reasons: 1) Omly Refseq accessions were retrieved i.e. `refseq` provided to `-s` flag; 2) Both Refseq and Genbank accessions were retrieved but the Genbank was not included in accessions_filtered.tsv, most likely because annotation details differed between accessions e.g. the Refseq accession was annotated as complete but not the Genbank accessions, meaning the Genbank accession was filtered as incomplete.<br>
<br>
